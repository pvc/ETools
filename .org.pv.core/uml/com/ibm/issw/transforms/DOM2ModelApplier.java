 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class DOM2ModelApplier implements Executable {
	...
	EMap properties;
	URI modelURI=null;
	Bundle b=Platform.getBundle("com.ibm.issw.RSAToolkit");
...
//	URI profileURI=URI.createURI(url.toString());
	 ...
	 * @param o
	 */
	public DOM2ModelApplier() {
		this.m=UMLFactory.eINSTANCE.createModel();
	}
	public DOM2ModelApplier(Document doc, String modelPath, String profilePath){
	...
	public NamedElement execute() {
		...
		processNode(d,null); // discard doc element
	...
	public void processNode(Node node,NamedElement parent) {
		boolean match=false;
		utils.log("***Processing Element Type: "+node.getNodeName());
		...
		if (node==d) {
			...
			if ((umlName!=null)) {
				...
				//				} 
				//				utils.log("*");
			} else {umlName=node.getLocalName()+getUid();}
			//			utils.log("Processing doc element: "+node.getNodeName());
			//		String ns=node.getNamespaceURI();  
			utils.log("UML Name="+umlName);
			
			String ns=null;
			utils.log(name);
			if ("patternApp".equals(name)) {
				ns="PBEProfile";
				NamedElement temp = m.getOwnedMember(umlName);
				utils.log(temp);
				if (temp==null) {root=m.createNestedPackage(umlName);}
//				if (temp==null) {root=(Package)m.createOwnedMember(UMLPackage.eINSTANCE.getPackage()); root.setName(umlName);}
			...
			EClass target = UMLPackage.eINSTANCE.getClass_();
			//			utils.log(s);
			if (s==null) {if ("Attribute".equals(name)) {target = UMLPackage.eINSTANCE.getProperty();} } else {
				EList exts = s.getAllExtendedMetaclasses();
				if (exts.isEmpty()) {utils.log("Error -No target defined on stereotype");} else {
					target = (EClass)exts.toArray()[0];
					//					utils.log("Metatype set to "+target.getName());
				}
			}
			
			utils.log("Metatype="+target.getName());
			boolean showAsProperty=false;
			if (target==UMLPackage.eINSTANCE.getProperty()) {
				showAsProperty=true;
					...
					if (parent==null) {match=true;} else {
						...
						for (Iterator iter = dps.iterator(); iter.hasNext();) {
							...
							for (Iterator it2 = sps.iterator(); it2.hasNext();) {
								...
								utils.log("Checking for match with existing element: "+ne.getName());
