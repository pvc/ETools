 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class DOM2ModelApplierOld implements Executable {
...
//	Stereotype stereotype;
	 ...
	 * @param o
	 */
	public DOM2ModelApplierOld(Model m) {
		this.m=m;
	}
...
//		String ns=node.getNamespaceURI();  
		...
		if (s==null) {utils.log("Error: Required Stereotype does not exist: "+node.getNodeName());}
			...
			if (umlName!=null) {c=(Class)m.getOwnedMember(umlName);} 
			else {umlName=getUid();}
			if (c==null) {
				c = (Class)m.createOwnedClass(umlName,false);
			}
