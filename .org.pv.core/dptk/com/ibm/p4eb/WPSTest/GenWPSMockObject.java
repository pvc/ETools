 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class GenWPSMockObject implements Executable  {
	...
	public Object execute(Object o) {
		...
		if ((f==null) || !("import".equalsIgnoreCase(f.getFileExtension()) ) )  {utils.log("Please select an import"); return null;}
		Document srcDoc = utils.getDoc((IFile)f);
		...
		String moName=fName;
		app.setAttribute("name",moName);
		app.setAttribute("export",fName);
		app.setAttribute("import",fName);
		Element root=(Element)srcDoc.getFirstChild();
		
		NodeList ifs = srcDoc.getElementsByTagName("interface");
		for (int i = 0; i < ifs.getLength(); i++) {
			...
			} catch (URISyntaxException e) {utils.log("Problem with URI in import file: "+a.getValue());
			...
			try {
				...
				if (wsdlFile==null) {
					IProject[] refs=f.getProject().getReferencedProjects();
					for (int j = 0; j < refs.length; j++) {
						utils.p("Trying project: "+refs[j]);
						wsdlFile=(IFile)refs[j].findMember(port[1].concat(".wsdl"));
						if (wsdlFile!=null) {break;}
					}
				}
...
//		utils.edit(appdef);
		utils.execDPTK(tgtDoc,"com.ibm.pv.patterns.SCAMockObject");
		Element bdg;
		NodeList bdgs=srcDoc.getElementsByTagName("esbBinding");
		if (bdgs.getLength()==0) {bdg=utils.addElement(srcDoc.getFirstChild(),"esbBinding");} else {bdg=(Element)bdgs.item(0);}
		bdg.setAttribute("xsi:type","scdl:SCAImportBinding");
		bdg.setAttribute("module",moName);
		bdg.setAttribute("export",moName);
		utils.save(srcDoc,(IFile)f);
		return null;
	...
	private void processWSDL(Element ife) {
		NodeList ports = d.getElementsByTagNameNS(wsdl,"portType");
		for (int portNo = 0; portNo < ports.getLength(); portNo++) {
			processPortType((Element)ports.item(portNo));
		}
		return;
	}	
	private void processPortType(Element portType) {	
		NodeList ops = portType.getElementsByTagNameNS(wsdl,"operation");
	...
	private void processElement(Element e) {
		...
		NodeList msgs = d.getElementsByTagNameNS(wsdl,"message");
		Element theMsg=null;
		...
		NodeList elts = d.getElementsByTagNameNS(xsd,"element");
		Element theElt=null;
